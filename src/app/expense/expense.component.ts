...
export class ExpenseComponent implements OnInit {
  ...
  fetchData(): void {
    ...
    if (loginId) {
      ...
      this.dataService.getFormDataByLoginId(parseInt(loginId, 10)).subscribe(
        (data) => {
          this.data = data;
          this.expenses = this.data.filter((item) => item.category === 'Expense');
          this.groupData();
  ...
  groupData(): void {
    this.groupedExpenses = this.expenses.reduce((acc, expense) => {
      ...
      acc[dateKey].total += expense.expenseAmount;
 ...
 this.groupedExpenses = Object.fromEntries(
  Object.entries(this.groupedExpenses).sort(([a], [b]) => {
    const dateA = new Date(a.split('/').reverse().join('-'));
    const dateB = new Date(b.split('/').reverse().join('-'));
    return dateB.getTime() - dateA.getTime();
  })
);
this.totalExpense = Object.values(this.groupedExpenses).reduce((sum: number, item: any) => sum + item.total, 0);
