...
export class SavingComponent  implements OnInit {
  ...
  fetchData(): void {
    ...
    if (loginId) {
      ...
      this.dataService.getFormDataByLoginId(parseInt(loginId, 10)).subscribe(
        (data) => {
          this.data = data;
          this.Savings = this.data.filter((item) => item.category === 'Saving');
          this.groupData();
  ...
  getSavingKeys(): string[] {
    return Object.keys(this.groupedSavings);
  }
  groupData(): void {
    this.groupedSavings = this.Savings.reduce((acc, Saving) => {
      let dateKey = '';
      ...
      acc[dateKey].total += Saving.savingAmount;
      acc[dateKey].items.push(Saving);
      return acc;
    }, {});
// 🔹 **Sort keys in descending order (latest first)**
this.groupedSavings = Object.fromEntries(
 // Object.entries(this.groupedSavings).sort(([a], [b]) => new Date(b).getTime() - new Date(a).getTime())
 Object.entries(this.groupedSavings).sort(([a], [b]) => {
  const dateA = new Date(a.split('/').reverse().join('-'));
  const dateB = new Date(b.split('/').reverse().join('-'));
  return dateB.getTime() - dateA.getTime();
})
);
this.totalSaving = Object.values(this.groupedSavings).reduce((sum: number, item: any) => sum + item.total, 0);
